...
class ExtractDataFromFile:
    ...
    def sumFeatures(self, window, feature):
        sum_feature = 0
        for host in window:
            sum_feature += window[host][feature]
        return sum_feature
    ...
    def extractPower(self):
        ...
        for i in range(len(self.powerLinear[0])):
            sum = 0
            for id_mote in self.powerLinear:
                sum += self.powerLinear[id_mote][i]
            sum /= len(self.powerLinear[0])
            listP.append(sum)
            listT.append(self.timeframe + listT[-1])
        axpower.plot(listT, listP, label=self.file.split("/")[-2])
        axpower.legend()
        plt.title("Power")
    ...
    def extractData(self):
        ...
        for window in self.packetSummary:
            for host in window:
                ...
                # new ratio : all DIS / DIS per Host
                if window[host]["dis"] == 0:
                    window[host]["dis"] = 1
                if window[host]["dio"] == 0:
                    window[host]["dio"] = 1
                if window[host]["dao"] == 0:
                    window[host]["dao"] = 1
                if window[host]["rank-changes"] == 0:
                    window[host]["rank-changes"] = 1
                if window[host]["version-changes"] == 0:
                    window[host]["version-changes"] = 1

                window[host]["dis"] = self.sumFeatures(window, "dis") / window[host]["dis"]
                window[host]["dio"] = self.sumFeatures(window, "dio") / window[host]["dio"]
                window[host]["dao"] = self.sumFeatures(window, "dao") / window[host]["dao"]
                window[host]["rank-changes"] = self.sumFeatures(window, "rank-changes") / window[host]["rank-changes"]
                window[host]["version-changes"] = self.sumFeatures(window, "version-changes") / window[host]["version-changes"]
                if host not in hosts:
                    hosts.append(host)


        cpt = 0
        #attempt to remove the "noise" from the vector, I try to remove all the vector that do not have significant number
        for window in self.packetSummary:
            for host in list(window):
                if window[host]["dis"] <= 2 and window[host]["dio"] <= 2 and window[host]["dao"] <= 2 and window[host]["version-changes"] <= 2  and window[host]["rank-changes"] <= 2:
                    window.pop(host)
                    cpt+=1
        print("Erreur:",cpt)
...
class ProcessFiles:
    def __init__(self, files):
        ...
        self.normData = []
        self.normDataArray = []
        self.listOfColors = ["blue", "orange", "green", "red"]
        self.colors = []
